 ...
 *
 * Keys may also live in <agentDir>/secrets.env (KEY=value lines).
...
export function formatResults(query: string, provider: string, results: SearchResult[]): string {
	if (results.length === 0) {
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Web Search",
		description:
			"Search the public web by keyword. Returns titles, URLs, and short snippets. Prefer this for current events/docs discovery, then web_fetch useful URLs. Providers: Tavily (TAVILY_API_KEY / OPENPI_TAVILY_API_KEY), Brave (BRAVE_API_KEY), else DuckDuckGo (no key). Keys may also be in <agentDir>/secrets.env.",
