 ...
 *   watch    — add | list | remove repos in <agentDir>/github-watch.json
 *   status   — watch list + auth state
 *
 * Auth: GITHUB_TOKEN / OPENPI_GITHUB_TOKEN in <agentDir>/secrets.env (or env).
...
export default function (pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: "GitHub 自动化",
		description:
			"GitHub automation: prs | issues | ci | comment | weekly | watch | status. Read PRs/issues/CI runs, post comments, and get a 7-day weekly summary across watched repos. Token: GITHUB_TOKEN / OPENPI_GITHUB_TOKEN in <agentDir>/secrets.env (comment requires one).",
		promptSnippet:
					...
					if (!githubToken()) {
						return {
							content: [
								{
									type: "text",
									text: "github comment requires GITHUB_TOKEN / OPENPI_GITHUB_TOKEN in <agentDir>/secrets.env.",
								},
							],
