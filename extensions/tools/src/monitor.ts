 ...
 *
 * State lives in <agentDir>/monitor.json. RSS items are tracked by URL;
...
export default function (pi: ExtensionAPI): void {
	pi.registerTool({
		...
		label: "订阅监控",
		description:
			"Watch RSS/Atom feeds and web pages for updates: add | list | remove | check | summary. check reports new feed items and changed pages since the last run; summary renders them as a markdown digest (每日摘要). State in <agentDir>/monitor.json.",
