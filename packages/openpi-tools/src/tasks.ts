/**
 * Session todo list (in-chat checklist) — NOT OpenPI Desktop scheduled/cron tasks.
 *
 * For 定时任务 / daily cron / unattended agent jobs, use the `scheduled_task` tool
 * (orchestrator → Desktop "定时任务" UI). This tool only tracks todos inside one session.
 *
 * - Registers a `tasks` tool for session-scoped todo CRUD
 * - Registers a `/tasks` command
 * - Persists via session entries (fork/branch aware)
 */
...
const TasksParams = Type.Object({
	...
	title: Type.Optional(
		Type.String({
			description: "Todo title (required for create). Not a cron job — use scheduled_task for 定时任务.",
		}),
	),
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Session Todos",
		description:
			"Session-scoped todo checklist (priorities, labels, status). Actions: list, create, update, complete, delete, clear. NOT for cron/定时任务 — use scheduled_task for OpenPI Desktop scheduled jobs.",
		promptGuidelines: [
			"Use tasks only for in-session checklists while implementing something now.",
			"For 定时任务 / daily cron / unattended runs that must show in Desktop 定时任务, use scheduled_task instead.",
			"Mark todos in_progress when you start, done when finished.",
		],
