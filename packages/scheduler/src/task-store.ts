...
import { runsPath, stepRunsPath, tasksPath } from "./config.ts";
...
/**
 * Read a store file, quarantining it if it will not parse.
 *
 * A corrupt file used to throw out of `loadTasks()`, which runs on every tick
 * and at startup - so one bad write took the whole scheduler down. Returning
 * undefined instead would be worse in a different way: the next save would
 * overwrite the damaged file and destroy whatever was still recoverable. So the
 * file is renamed aside and the caller continues from empty.
 */
function readJson(path: string): unknown {
	...
	try {
		...
		try {
			...
			// Even the rename failed; there is nothing further to try.
		}
		return undefined;
	}
}
