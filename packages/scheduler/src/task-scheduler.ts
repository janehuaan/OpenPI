...
interface ActiveRun {
	runId: string;
	/** Cancels the run: one process for a single-step task, every in-flight step otherwise. */
	cancel: () => void;
	cancelled: boolean;
}
...
export class TaskScheduler {
	...
	private async executeSingleStepTask(task: TaskDefinition, run: TaskRun): Promise<TaskRun> {
		try {
			...
			const active: ActiveRun = { runId: run.id, cancel: () => execution.cancel(), cancelled: false };
	...
	private async executeStepTask(task: TaskDefinition, run: TaskRun, trigger: TaskRun["trigger"]): Promise<TaskRun> {
		...
		try {
			// No parent process in step mode: each step spawns its own. Cancelling a
			// step task means cancelling whichever steps are in flight, so the handle
			// registered here forwards to them.
			...
			while (completed.size + skipped.size + failed.size < steps.length) {
				...
				for (const step of ready) {
					...
					// Record the log paths now: the step's result is read back from
					// stdout when it finishes, and without this it stays empty.
					this.store.updateStepRun(stepRun.id, (r) => ({
						...r,
						pid: stepExec.pid,
