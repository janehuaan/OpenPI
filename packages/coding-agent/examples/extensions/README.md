...
| `security-gate.ts` | **NEW** — Comprehensive permission/approval system with configurable security levels, audit logging, and git dirty checking |
...
| `tasks.ts` | **NEW** — Structured task management with priorities, labels, status tracking, and filtering |
| `memory/` | **NEW** — File-based memory system (4 types, index + topic files, frozen snapshot) |
| `tasks.ts` | **NEW** — Structured task management with priorities, labels, status tracking, and filtering |
| `hello.ts` | Minimal custom tool example |
| `web-fetch.ts` | **NEW** — Web fetch tool for retrieving page content from arbitrary URLs |
...
| `memory/` | **NEW** — File-based memory system (4 types, index + topic files, frozen snapshot) |
| `code-search.ts` | **NEW** — Code symbol/keyword search with language filtering and ripgrep backend |
| `memory/` | **NEW** — File-based memory system (4 types, index + topic files, frozen snapshot) |
