 ...
 *
 * Provider priority: Tavily (TAVILY_API_KEY / OPENPI_TAVILY_API_KEY) →
 * Brave (BRAVE_API_KEY / OPENPI_BRAVE_API_KEY) → DuckDuckGo HTML (no key).
...
async function searchTavily(query: string, maxResults: number, signal?: AbortSignal): Promise<SearchResult[]> {
	...
	const response = await fetch("https://api.tavily.com/search", {
		...
		headers: {
			...
			"User-Agent": "pi-coding-agent/0.80.8",
		},
		body: JSON.stringify({
			api_key: key,
			query,
...
export function createWebSearchToolDefinition(): ToolDefinition<typeof WebSearchParams, WebSearchDetails | undefined> {
	return {
		name: "web_search",
		label: "Web Search",
		description:
		...
		async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
			...
			if (!query) {
				return webSearchError("query is required", query);
			}
			...
			try {
				...
				if (!results) {
					try {
						...
						return webSearchError(errors.join(" | ") || "All search providers failed", query);
					}
				}
				...
				return webSearchError(message, query);
			} finally {
...
function webSearchError(message: string, query: string): AgentToolResult<WebSearchDetails | undefined> {
	return {
		content: [{ type: "text", text: `Error searching the web for "${query}": ${message}` }],
