...
- Runtime tools (`sub_agent`, `parallel_tasks`, `submit_job`, `wait_job`, `list_jobs`) are now active by default in the SDK (previously omitted from the default activation list).
- `code_index` gained `refs` (cross-file reference search, marking definition lines) and `callers` (nearest enclosing function/method for each reference) actions.
- Background jobs persist across turns and are reconciled at session startup: jobs left in `running` by a previous process are marked failed (`interrupted by process restart`); `list_jobs` shows every job's status.
- `session_search` reranking no longer requires an embedding API key: without `OPENPI_EMBEDDING_API_KEY` it uses a zero-dependency local hashing embedding (word tokens + char bigrams into a 256-dim vector).
- `session_search` now queries a persistent local vector library (`<cwd>/.pi/vector-store.json`): memory entries and session messages are vectorized offline (incremental, content-fingerprint dedupe, 5000-doc cap), and retrieval is a full cosine scan blended with normalized BM25 (`0.7·cosine + 0.3·bm25`). `rerank: off` keeps the pure BM25 path.
- Local embedding model (zero network, no API key): `OPENPI_EMBEDDING_LOCAL=1` spawns a bundled llama.cpp `llama-server` with a 25 MB bge-small-zh (Chinese-friendly, 512-dim) GGUF model (`<app>/Resources/embedding/`, or `OPENPI_EMBEDDING_BIN`/`OPENPI_EMBEDDING_MODEL_FILE`). The vector library uses it for real semantic embeddings; falls back to the hashing embedder when unavailable.
- The local embedding model is now **built in by default**: when the bundled llama-server + model are present (desktop package), `session_search` and the vector library use them without any env var (`OPENPI_EMBEDDING_LOCAL=0` opts out).
- Prompt-cache optimization: the task list (`todo_write`/`complete_step`) moved out of the system prompt into a **message injected only when its content changes**, placed before the current user message — todo updates now invalidate only the history tail instead of the whole prefix, so long-session provider cache hits stay high (simulation: ≥99% hit in long plain-session tails).
- Conversation-level cache telemetry: `agent_end` prints `[cache-session] <avg%> hit · tokens · requests this conversation` so cache behavior is verifiable end-to-end.
- Next-turn memory injection (openpi-memory proactive snapshot) is deduplicated: identical customType+content is injected once per session, not every turn — eliminating a per-turn cache write that previously capped long-session hit rates near 99.1% (now 99.8%+ in the later third of 150+-turn sessions, per the cache simulation).
...
- Manually verified metadata aliases for private model ids that public databases don't cover (e.g. `agnes-2.5-pro-alpha` / `agnes-2.5-pro` → 1M context / $0.45-$0.9 per M tokens, verified via Artificial Analysis + live probing + user confirmation); priority above models.dev/LiteLLM.
